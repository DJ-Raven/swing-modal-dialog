...
public class MultiXYTextAnnotation extends AbstractXYAnnotation {
    ...
    public void draw(Graphics2D g, XYPlot plot, Rectangle2D dataArea, ValueAxis domainAxis, ValueAxis rangeAxis, int rendererIndex, PlotRenderingInfo info) {
        ...
        if (labels != null) {
            ...
            int shadow = 5;
            int space = 2;
            Rectangle2D rec = ChartUtils.scaleRectangleInsets(space, space, space + shadow, space + shadow).createInsetRectangle(dataArea);

            Rectangle2D bgRecScale = ChartUtils.scaleRectangle(bgRec);
            if (!rec.contains(bgRecScale)) {
                ...
                bgRec = ChartUtils.unscaleRectangle(new Rectangle2D.Double(x, y, width, height));
            }
            Shape shape = round > 0 ?
